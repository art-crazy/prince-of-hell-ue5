...
The only approved production animation target is
`SK_POHPrince_AccuRig` with `SK_POHPrince_AccuRig_Skeleton`. Assets below
`NativeTripo`, `TripoRig` and `RetargetedTemplateAligned` are historical
diagnostics and must not be used for new gameplay work. Direct Manny animation
assignment and Blender rotation-copying remain prohibited.
...
The isolated production path now exists at
...
- `ABP_POH_AccuRig_UE58` — UE 5.8's authored idle/walk/run/jump/fall/land
  state graph copied into an AccuRIG-bound AnimBP. Its five animation
  dependencies are replaced with the matching outputs in
  `RetargetedUE58_ABP`; it has no direct Manny animation references.
- `GM_POH_AccuRigAnimGraph` and `Lvl_POH_AccuRigAnimGraph_QA` — an isolated
  GameMode and map for human visual QA. The existing playable map is unchanged.

`PrinceAnimationRuntime` detects Animation Blueprint mode and deliberately
does not overwrite this pawn with its historical Single Node diagnostic path.
It only applies the bounds-derived AccuRIG placement once at spawn. The QA
gate is visual: idle, walk/run, jump, fall and land must remain visible and
grounded before this GameMode is promoted beyond its isolated map.

The build is reproducible with:

1. `CreateAccuRigUE58AnimGraph.py` — duplicates the authored UE graph
   and binds it to the AccuRIG skeleton.
2. `RebindAccuRigUE58AnimGraph.py` — replaces every graph animation reference
   with the corresponding `UE58_` AccuRIG asset.
3. `ValidateAccuRigUE58AnimGraph.py` and
   `ValidateAccuRigAnimGraphQaMap.py` — validate skeleton, references,
   Animation Blueprint mode, pawn and map wiring without rendering a scene.

The NativeTripo retargeters remain quarantined as historical evidence only.
